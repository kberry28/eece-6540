...
int main()
{
    ...
    size_t local_size;
    int group_int = local_size*1;
    int total_terms=128;
    int floats_per_item=4;
    /*this gives the total number of work items*/
    size_t global_size=total_terms/floats_per_item;
...
#error "unknown OpenCL SDK environment"
    ...
    printf("Number of global buckets is: %i\n",buckets);
    ...
    CL_MEM_COPY_HOST_PTR, sizeof(result), &result, NULL);
     if (ret != CL_SUCCESS) {
      printf("Failed to create result buffer.\n");
      exit(1);
    }
    ...
    ret = clSetKernelArg(kernel, 0, sizeof(floats_per_item), &floats_per_item);
        if(ret < 0) {
       printf("Couldn't set kernel argument 0");
       exit(1);
    }
    /*how many work items are in the current work group*/
    ret |= clSetKernelArg(kernel, 1, sizeof(group_int), &group_int);
            if(ret < 0) {
       printf("Couldn't set kernel argument 1");
       exit(1);
    }
    /*local memory array to hold summation from each work item in the work group*/
    ret |= clSetKernelArg(kernel, 2, group_int * sizeof(float), NULL);
            if(ret < 0) {
       printf("Couldn't set kernel argument 2");
       exit(1);
    }
   	
    ret |= clSetKernelArg(kernel, 3, sizeof(cl_mem), &result_buffer);
    if(ret < 0) {
       printf("Couldn't set kernel argument 3");
       exit(1);
    }
