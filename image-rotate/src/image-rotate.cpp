    ...
    float cosTheta, const size_t ImageRows, const size_t ImageCols) 
{
  ...
  q.submit([&](handler &h) {
    ...
    { 
      ...
      int row = item[0];  //y position
      int col = item[1];  //x position
      /* calculate location of data to move int (row, col)
      * output decomposition as mentioned*/
      float xpos = ((float)col)*cosTheta - ((float)(ImageRows-row))*sinTheta;
      float ypos = ((float)col)*sinTheta + ((float)(ImageRows-row))*cosTheta;
      //bmp adjustment
      //float ypos_adj = ImageRows - ypos;
       ...
       if(((int)xpos >= 0) && ((int)xpos < ImageCols) &&
          ((int)(ImageRows - ypos) >= 0) && ((int)(ImageRows - ypos) < ImageRows) )
       {
           ...
           * write its results independently*/
           dstPtr[(int)(ImageRows - ypos) * ImageCols + (int)xpos] = srcPtr[row*ImageCols+col];
       }
    }); //end of parallel_for
  });   //end of q.submit
}
